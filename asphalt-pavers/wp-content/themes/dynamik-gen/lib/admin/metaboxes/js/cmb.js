...
jQuery(document).ready(function ($) {
	 ...
	 */
	$('.dynamik_cmb_timepicker').each(function () {
	 ...
	 */
	$('.dynamik_cmb_datepicker').each(function () {
	...
	$("#ui-datepicker-div").wrap('<div class="dynamik_cmb_element" />');

	/**
	 * Initialize color picker
	 */
	if (typeof jQuery.wp === 'object' && typeof jQuery.wp.wpColorPicker === 'function') {
		$('input:text.dynamik_cmb_colorpicker').wpColorPicker();
	} else {
		$('input:text.dynamik_cmb_colorpicker').each(function (i) {
	 ...
	 */
	$('.dynamik_cmb_upload_file').change(function () {
		formfield = $(this).attr('name');
		$('#' + formfield + '_id').val("");
	});

	$('.dynamik_cmb_upload_button').live('click', function () {
		var buttonLabel;
	...
	$('.dynamik_cmb_remove_file_button').live('click', function () {
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if (itemurl.match(image)) {
				uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="dynamik_cmb_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
			} else {
				// No output preview if it's not an image
				// Standard generic output if it's not an image.
				html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
				uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="dynamik_cmb_remove_file_button" rel="' + formfield + '">Remove</a></div>';
			}

			$('#' + formfield).val(itemurl);
			$('#' + formfield + '_id').val(itemid);
			$('#' + formfield).siblings('.dynamik_cmb_media_status').slideDown().html(uploadStatus);
			tb_remove();
	...
	// ajax on paste
	$('.dynamik_cmb_oembed').bind('paste', function (e) {
		var pasteitem = $(this);
		// paste event is fired before the value is filled, so wait a bit
		setTimeout(function () {
			// fire our ajax function
			doDYNAMIKCMBajax(pasteitem, 'paste');
		}, 100);
	}).blur(function () {
		// when leaving the input
		setTimeout(function () {
			// if it's been 2 seconds, hide our spinner
			$('.postbox table.dynamik_cmb_metabox .dynamik-cmb-spinner').hide();
		}, 2000);
	});

	// ajax when typing
	$('.dynamik_cmb_metabox').on('keyup', '.dynamik_cmb_oembed', function (event) {
		// fire our ajax function
		doDYNAMIKCMBajax($(this), event);
	});

	// function for running our ajax
	function doDYNAMIKCMBajax(obj, e) {
		// get typed value
		...
		if (e === 'paste' || e.which <= 90 && e.which >= 48 || e.which >= 96 && e.which <= 111 || e.which == 8 || e.which == 9 || e.which == 187 || e.which == 190) {
			...
			var context = obj.parents('.dynamik_cmb_metabox tr td');
			// show our spinner
			$('.dynamik-cmb-spinner', context).show();
			// clear out previous results
			$('.embed_wrap', context).html('');
			// and run our ajax function
			setTimeout(function () {
				// if they haven't typed in 500 ms
				if ($('.dynamik_cmb_oembed:focus').val() == oembed_url) {
					$.ajax({
						type : 'post',
						dataType : 'json',
						url : window.ajaxurl,
						data : {
							'action': 'dynamik_cmb_oembed_handler',
							'oembed_url': oembed_url,
							'field_id': field_id,
							'post_id': window.dynamik_cmb_ajax_data.post_id,
							'dynamik_cmb_ajax_nonce': window.dynamik_cmb_ajax_data.ajax_nonce
						},
						success: function (response) {
							// if we have a response id
							if (typeof response.id !== 'undefined') {
								// hide our spinner
								$('.dynamik-cmb-spinner', context).hide();
